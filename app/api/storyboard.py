...
async def create_storyboard(
    ...
    생성된 이미지는 콘티 썸네일 겸 Veo 영상 생성 시 시작 프레임으로 사용됩니다.
    ...
    # 캐릭터 설명 조회
    try:
        char_desc = await get_character_description(req.character_id, req.custom_character_id)
...
async def regenerate_scene_image(
    ...
    # 캐릭터 설명 조회
    try:
        char_desc = await get_character_description(sb.characterId, sb.customCharacterId)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from None
